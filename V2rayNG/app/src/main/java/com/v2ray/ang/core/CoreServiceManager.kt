...
//        LogUtil.d(AppConfig.TAG, "Queried outbound traffic stats: $result")
    ...
    private fun measureV2rayDelay() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (time < 0) {
                LogUtil.w(AppConfig.TAG, "StartCore-Manager: Delay probe unavailable: $errorStr")
                MessageUtil.sendMsg2UI(service, AppConfig.MSG_MEASURE_DELAY_SUCCESS, service.getString(R.string.connection_connected))
                return@launch
            }

            val result = service.getString(R.string.connection_test_available, time)

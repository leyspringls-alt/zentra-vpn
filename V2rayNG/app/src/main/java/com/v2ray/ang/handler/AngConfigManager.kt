...
object AngConfigManager {
    ...
    fun importBatchConfig(server: String?, subid: String, append: Boolean): Pair<Int, Int> {
        val normalizedServer = Utils.normalizeConfigInput(server)

        var count = parseBatchConfig(Utils.decode(normalizedServer), subid, append)
        if (count <= 0) {
            count = parseBatchConfig(normalizedServer, subid, append)
        }
        if (count <= 0) {
            count = parseCustomConfigServer(normalizedServer, subid, append)
        }

        var countSub = parseBatchSubscription(normalizedServer)
        if (countSub <= 0) {
            countSub = parseBatchSubscription(Utils.decode(normalizedServer))
        }
        if (countSub > 0) {

...
class SettingsActivity : BaseActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        ...
        companion object {
            private const val PREF_ZENTRA_PER_APP = "pref_zentra_per_app"
        }
        ...
        private val zentraSplitMode by lazy { findPreference<ListPreference>(AppConfig.PREF_ZENTRA_SPLIT_MODE) }
        private val zentraPerApp by lazy { findPreference<Preference>(PREF_ZENTRA_PER_APP) }
        ...
        override fun onCreatePreferences(bundle: Bundle?, s: String?) {
            ...
            zentraSplitMode?.setOnPreferenceChangeListener { pref, newValue ->
                val valueStr = newValue.toString()
                (pref as? ListPreference)?.let { lp ->
                    val idx = lp.findIndexOfValue(valueStr)
                    lp.summary = if (idx >= 0) lp.entries[idx] else valueStr
                }
                context?.let { SettingsManager.applySplitTunnelMode(it, valueStr) }
                activity?.toastSuccess(R.string.zentra_split_mode_applied)
                true
            }

            // Раздельное туннелирование: приложения
            zentraPerApp?.setOnPreferenceClickListener {

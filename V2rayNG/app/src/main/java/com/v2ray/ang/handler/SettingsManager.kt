...
object SettingsManager {
    ...
    private const val ZENTRA_SETTINGS_VERSION_KEY = "zentra_settings_version"
    private const val ZENTRA_SETTINGS_VERSION = 13

    /** Индексы пресетов в RoutingType для режимов раздельного туннелирования. */
    private const val PRESET_INDEX_FULL_VPN = 2
    private const val PRESET_INDEX_RU_DIRECT = 4

    @Volatile
    private var runtimeSocksPort: Int? = null

    fun initApp(context: Context) {
        val applyZentraMigration = needsZentraSettingsMigration()
        ensureDefaultSettings(applyZentraMigration)
        forceZentraRuntimeSettings()
        //ensureDefaultSubscription()
        initRoutingRulesets(context, applyZentraMigration)
     ...
     */
    private fun initRoutingRulesets(context: Context, forceReset: Boolean = false) {
        val exist = MmkvManager.decodeRoutingRulesets()
        if (forceReset || exist.isNullOrEmpty()) {
            applySplitTunnelMode(context, getSplitTunnelMode())
        }
    }

    /**
     * Текущий режим раздельного туннелирования сайтов.
     */
    fun getSplitTunnelMode(): String {
        return MmkvManager.decodeSettingsString(AppConfig.PREF_ZENTRA_SPLIT_MODE)
            ?.takeIf { it.isNotEmpty() }
            ?: AppConfig.ZENTRA_SPLIT_MODE_FULL
    }

    /**
     * Применяет режим раздельного туннелирования: перезаписывает набор правил
     * маршрутизации соответствующим пресетом и подбирает стратегию доменов.
     *
     * @param context The application context.
     * @param mode ZENTRA_SPLIT_MODE_FULL или ZENTRA_SPLIT_MODE_RU_DIRECT.
     */
    fun applySplitTunnelMode(context: Context, mode: String) {
        val presetIndex = when (mode) {
            AppConfig.ZENTRA_SPLIT_MODE_RU_DIRECT -> PRESET_INDEX_RU_DIRECT
            else -> PRESET_INDEX_FULL_VPN
        }
        // Для правил по geoip/geosite домен нужно резолвить, иначе IP-правила не сработают.
        val domainStrategy = when (mode) {
            AppConfig.ZENTRA_SPLIT_MODE_RU_DIRECT -> "IPIfNonMatch"
            else -> "AsIs"
        }

        MmkvManager.encodeSettings(AppConfig.PREF_ZENTRA_SPLIT_MODE, mode)
        MmkvManager.encodeSettings(AppConfig.PREF_ROUTING_DOMAIN_STRATEGY, domainStrategy)

        val rulesetList = getPresetRoutingRulesets(context, presetIndex) ?: return
    ...
    /**
     * Zentra uses a strict full-device VPN profile.
     */
    ...
    private fun forceZentraRuntimeSettings() {
        MmkvManager.encodeSettings(AppConfig.PREF_MODE, VPN)
        // PREF_PER_APP_PROXY / PREF_BYPASS_APPS / PREF_ROUTING_RULESET здесь НЕ сбрасываются:
        // это настройки раздельного туннелирования, которыми управляет пользователь.
        ...
        MmkvManager.encodeSettings(AppConfig.PREF_VPN_MTU, "1280")
        ...
        MmkvManager.encodeSettings(AppConfig.PREF_IPV6_ENABLED, false)
        ...
        MmkvManager.encodeSettings(AppConfig.PREF_MUX_XUDP_QUIC, false)
        ...
        MmkvManager.encodeSettings(AppConfig.PREF_REMOTE_DNS, "1.1.1.1,8.8.8.8")
        MmkvManager.encodeSettings(AppConfig.PREF_DOMESTIC_DNS, "1.1.1.1,8.8.8.8")
        MmkvManager.encodeSettings(AppConfig.PREF_DELAY_TEST_URL, AppConfig.DELAY_TEST_URL)
    }
    private fun ensureDefaultSettings(applyZentraMigration: Boolean) {
        if (applyZentraMigration) {
            MmkvManager.encodeSettings(AppConfig.PREF_MODE, VPN)
            ...
            MmkvManager.encodeSettings(AppConfig.PREF_REMOTE_DNS, "1.1.1.1,8.8.8.8")
            MmkvManager.encodeSettings(AppConfig.PREF_DOMESTIC_DNS, "1.1.1.1,8.8.8.8")
            MmkvManager.encodeSettings(ZENTRA_SETTINGS_VERSION_KEY, ZENTRA_SETTINGS_VERSION.toString())
        }

        // Zentra defaults: весь трафик через VPN; раздельное туннелирование включает пользователь.
        ensureDefaultValue(AppConfig.PREF_MODE, VPN)
        ensureDefaultValue(AppConfig.PREF_ZENTRA_SPLIT_MODE, AppConfig.ZENTRA_SPLIT_MODE_FULL)
        ensureDefaultValue(AppConfig.PREF_VPN_DNS, "1.1.1.1,8.8.8.8")
        ensureDefaultValue(AppConfig.PREF_VPN_MTU, "1280")
        ensureDefaultValue(AppConfig.PREF_SOCKS_PORT, AppConfig.PORT_SOCKS)
        ensureDefaultValue(AppConfig.PREF_REMOTE_DNS, "1.1.1.1,8.8.8.8")
        ensureDefaultValue(AppConfig.PREF_DOMESTIC_DNS, "1.1.1.1,8.8.8.8")
    ...
    private fun needsZentraSettingsMigration(): Boolean {
        val current = MmkvManager.decodeSettingsString(ZENTRA_SETTINGS_VERSION_KEY)?.toIntOrNull() ?: 0
        return current < ZENTRA_SETTINGS_VERSION
    }

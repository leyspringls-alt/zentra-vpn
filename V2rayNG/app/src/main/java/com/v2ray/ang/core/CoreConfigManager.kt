...
object CoreConfigManager {
    ...
    /**
     * Local DNS interception is disabled in Zentra full-VPN mode.
     */
    private fun configureLocalDns(v2rayConfig: V2rayConfig) {
        return
    }
    /**
     * Root-mode DNS interception is disabled because Zentra uses Android VPN mode only.
     */
    ...
    /**
     * Configure DNS servers and force DNS traffic through the selected VPN outbound.
     */
    private fun configureDns(
        ...
        val proxyDomain = collectCustomOutboundDomains().distinct()
        remoteDns.forEach {
            servers.add(it)
        }
        if (proxyDomain.isNotEmpty() && remoteDns.isNotEmpty()) {
            servers.add(
        ...
        // Keep these Android Private DNS aliases, but never route DNS direct.
        ...
        v2rayConfig.dns = V2rayConfig.DnsBean(
            ...
            enableParallelQuery = if (remoteDns.size > 2) true else null
        )
    ...
    /**
     * Configure one full-tunnel routing rule and ignore user split-tunnel rulesets.
     */
    private fun configureRouting(
        ...
        val fullTunnelRule = if (proxyBalancerTag != null) {
            V2rayConfig.RoutingBean.RulesBean(
                network = "tcp,udp",
        ...
        v2rayConfig.routing.rules.add(fullTunnelRule)
    }
    /**

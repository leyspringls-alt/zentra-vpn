...
object Utils {
    ...
    private val CONFIG_PROTOCOLS = listOf(
        AppConfig.VMESS,
        AppConfig.VLESS,
    ...
    /**
     * Accepts either a raw VLESS/etc. link or the Zentra portal link with #d= payload.
     */
    fun normalizeConfigInput(text: String?): String? {
        if (text.isNullOrBlank()) return text

        val decoded = urlDecode(text.trim())
        findConfigUri(decoded)?.let { return it }
        extractZentraShareUri(decoded)?.let { return it }
        return decoded
    }

    private fun extractZentraShareUri(value: String): String? {
        val payload = extractPayloadParam(value) ?: return null
        val jsonText = decodeBase64Payload(payload) ?: return null

        return try {
            val json = JSONObject(jsonText)
            var rawLink: String? = null
            for (key in listOf("nl", "url", "link", "vless")) {
            ...
            val candidateText = rawLink?.let { urlDecode(it) } ?: jsonText
            findConfigUri(candidateText) ?: candidateText.takeIf { it.isNotBlank() }
        } catch (_: Exception) {
            findConfigUri(jsonText) ?: jsonText.takeIf { it.isNotBlank() }
        }
    }

    private fun extractPayloadParam(value: String): String? {
        val fragment = value.substringAfter('#', "")
        extractNamedParam(fragment, "d")?.let { return it }
        return Regex("""(?:^|[?&#])d=([^&#\s]+)""").find(value)?.groupValues?.getOrNull(1)
    }

    private fun extractNamedParam(params: String, name: String): String? {
    ...
    private fun findConfigUri(value: String): String? {
        value.lines().forEach { line ->
            val trimmed = line.trim()
            if (CONFIG_PROTOCOLS.any { trimmed.startsWith(it, ignoreCase = true) }) {
                return trimmed
            }
        }

        return Regex(
            """(?:vmess|vless|ss|socks4?|socks5|trojan|wireguard|tuic|hysteria2?|hy2)://[^\s<>"']+""",
            RegexOption.IGNORE_CASE
        ).find(value)?.value
    }

    private fun decodeBase64Payload(payload: String): String? {
        var fixed = payload.trim()
        ...
        return try {
            Base64.decode(fixed, Base64.NO_WRAP.or(Base64.URL_SAFE)).toString(Charsets.UTF_8)
        } catch (_: Exception) {
            try {
                Base64.decode(fixed, Base64.NO_WRAP).toString(Charsets.UTF_8)
            } catch (_: Exception) {
                null
            }
        }
    ...
    fun decode(text: String?): String {
        ...
        return tryDecodeBase64(trimmed) ?: trimmed.trimEnd('=').let { tryDecodeBase64(it) }.orEmpty()
    }
    ...
    fun tryDecodeBase64(text: String?): String? {
        ...
        try {
            ...
            // Many imports are plain vless:// links, so failed base64 is a normal fallback path.
        }
        try {
            return Base64.decode(text, Base64.NO_WRAP.or(Base64.URL_SAFE)).toString(Charsets.UTF_8)
        } catch (_: Exception) {
            // Keep logs clean; the caller will try the original text next.
        }
        return null
    }
     ...
     */
    fun isXray(): Boolean = BuildConfig.APPLICATION_ID.startsWith("com.v2ray.ang") || BuildConfig.APPLICATION_ID.startsWith("com.zentra.vpn")
